...
import styles from './InsightsCategory.module.scss'
import SidebarSubscribe from '@/components/molecules/SidebarSubscribe'
import Button from '@/components/atoms/Button'
import { getInsightsCategory } from '@/functions/wordpress/fetchData'
import Ad from '@/components/molecules/Ad'
import Background from '@/components/atoms/Background'
import Image from '@/components/atoms/Image'
import PostCard from '@/components/molecules/PostCard'

export default function InsightsCategory({ category, community }) {
  const [page, setPage] = useState(1)
  const [isEnd, setIsEnd] = useState(false)
  const [posts, setPosts] = useState([])

  const { data, loading } = getInsightsCategory(category, page)
